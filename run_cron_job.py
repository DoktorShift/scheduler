...
async def save_job_execution(response: str, jobID: str, adminkey: str) -> None:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(
                f"pushdb: now pushing execution data to the database for jobID: {jobID}"
            )
...
async def get_job_by_id(jobID: str, adminkey: str):
    ...
    try:
        ...
        logger.error(
            f"[get_job_by_id]: Error trying to fetch data from db, check is LNBITS server running?: {e}"
        )
