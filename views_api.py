...
from .models import CreateJobData, UpdateJobData, Job, JobDetailed, JobFilters, LogEntry
...
async def api_get_testlog(
    job_id: str, info: WalletTypeInfo = Depends(require_admin_key)
) -> JobDetailed:
...
async def api_job_log_delete(
    id: str, info: WalletTypeInfo = Depends(require_admin_key)
) -> None:
...
async def api_job_entry_create(
    data: LogEntry, info: WalletTypeInfo = Depends(require_admin_key)
) -> bool:
...
async def api_scheduler_jobs_update(
    job_id: str, data: UpdateJobData, info: WalletTypeInfo = Depends(require_admin_key)
) -> JobDetailed:
...
async def api_scheduler_pause(job_id, status) -> JobDetailed:
